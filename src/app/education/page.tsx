    ...
    <div className="site-container py-12 sm:py-16">
      ...
      <div className="scroll-grid grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {educationPages.map((page) => {
          ...
          return (
            <Link key={page.slug} href={`/education/${page.slug}`} className="surface-card scroll-card group overflow-hidden border border-[#dfd4c4] bg-[#fbf8f2] transition-all duration-500 hover:-translate-y-1 hover:border-[#c9a45f]">
              <div className="relative aspect-[16/10] bg-[#efe5d6]">
                {image ? (
                  <Image src={image} alt={page.title} fill sizes="(min-width: 1024px) 33vw, 100vw" className="scroll-image object-cover" />
                ) : (
